...
public class STetromino implements Tetromino
{
  ...
  public STetromino( int r, int c )
  {
    ...
    for( int i = 0; i < 4; i++ )
    {
      ...
      tiles.get( i ).setFillColor( Specs.S_COLOR );
      tiles.get( i ).setFrameColor( Color.BLACK );
      tiles.get( i ).setSize( Specs.TILE_SIZE, Specs.TILE_SIZE );
    }
    
    config = new Vector< Configuration >( );
    config.add( new Configuration( 1, 0, 2, 0, 0, 1, 1, 1 ) );
    config.add( new Configuration( 0, 0, 0, 1, 1, 1, 1, 2 ) );
    config.add( new Configuration( 1, 0, 2, 0, 0, 1, 1, 1 ) );
    config.add( new Configuration( 0, 0, 0, 1, 1, 1, 1, 2 ) );
  ...
  //********************************************************************
  //checks the next config of tiles
  //********************************************************************
