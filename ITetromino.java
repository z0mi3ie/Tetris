...
public class ITetromino implements Tetromino
{
  ...
  public ITetromino( int r, int c )
  {
    ...
    for( int i = 0; i < 4; i++ )
    {
      ...
      tiles.get( i ).setFillColor( Specs.I_COLOR );
      tiles.get( i ).setFrameColor( Color.BLACK );
      tiles.get( i ).setSize( Specs.TILE_SIZE, Specs.TILE_SIZE );
    }
    
    config = new Vector< Configuration >( );
    config.add( new Configuration( 1, 0, 1, 1, 1, 2, 1, 3 ) );
    config.add( new Configuration( 0, 0, 1, 0, 2, 0, 3, 0 ) );
    config.add( new Configuration( 2, 0, 2, 1, 2, 2, 2, 3 ) );
    config.add( new Configuration( 0, 2, 1, 2, 2, 2, 3, 2 ) );
  ...
  public void deRotate( )
  {
    ...
    {
      System.out.println( currentConfigIndex );
      currentConfigIndex -= 1;
      currentConfig = config.get( currentConfigIndex );
      draw( );
    }
  ...
  /*==================================================================*/
  /**
   *  Get the tiles in the next config
   */ 
